...
function loadObjFile( url, fallback_color )
{
    ...
    // makes temp vertices and temp textures in order to calculateTangentSpace.
    let temp_vertices = []
    let temp_textures = []
    for ( let i = 0; i < vertex_ids.length; i++ )
    {
        ...
        temp_vertices.push( vertices[ vid ], vertices[ vid + 1 ], vertices[ vid + 2 ] )
        temp_textures.push(textures[tid], textures[tid+1])
    }

    let [tangents, bitangents] = calculateTangentSpace(temp_vertices, temp_textures);
...
// added parsing of the texture ids.
/**
...
// Loads an image from a given url and returns it.
...
function parseSceneNode( entry, parent )
{
    ...
    else if ( entry.type == 'object' )
    {
        ...
        // Loads an image from the urls in the texture and normal_map.
        const obj_texture = loadTextureFile(entry.texture);
        const obj_normal = loadTextureFile(entry.normal_map);
        let textures = [obj_texture, obj_normal]

        node = new ObjectNode( obj_content, name, parent, translation, rotation, scale, obj_material, textures)
...
// Loads the texture given a gl context and an image.
